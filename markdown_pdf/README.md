...
长图直接消费结构化摘要：标题与真实作者/主要发言人组成紧凑题头，编辑分区使用连续编号与细分隔线，关键数字、结果和结论变化可以获得 0–2 处精确重点，且只在与正文完全匹配、互不重叠时着色。长图与网页优先使用项目内置的 Noto Sans SC 可变字体，并采用更窄的阅读页边、更紧凑的行距与分区节奏。整体保持冷灰画布、白色阅读纸、石墨文字与克制朱橙色，不添加文章快照、虚构 LOGO 或固定免责声明。如果摘要超过 Chromium 的稳定单图高度，页面会直接提示缩短原文或降低详细程度。
