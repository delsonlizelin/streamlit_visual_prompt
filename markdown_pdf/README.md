...
提示词要求中性、直接和忠实：核心摘要跨章节重组信息，提供一个明确结论和 3–5 个不重复要点，适合日常阅读与转发；按章节梳理沿原文结构逐节提炼，长度随有效章节数量增加，适合报告和课程。讲述方式独立于内容结构：直接摘要保留必要术语和信息密度；零基础讲解面向没有相关背景的成年读者，补充原文能够支持的前置知识，解释术语，并展开每个主要观点的原因、过程、证据、结果和全文推理链。零基础讲解不再使用极短 ELI5 篇幅，也不会使用儿童化语气或用外部常识填补原文缺失的信息。输出只使用当前 PDF 渲染器稳定支持的 Markdown；原文没有明确结论时，模型不得替作者补出结论。
