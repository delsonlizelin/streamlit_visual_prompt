...
- 快速概览、核心摘要、按章节梳理、零基础讲解四种方式，并在界面中直接说明长度、结构和适用场景；
...
- 一键复制当前模式实际使用的系统规则、模式指令和语言要求，正文位置使用安全占位符。
...
提示词要求中性、直接和高信息密度：快速概览严格为 3–5 句无列表短文，适合先判断是否值得细读；核心摘要跨章节重组信息，提供一个明确结论和 3–5 个不重复要点，适合日常阅读与转发；按章节梳理沿原文结构逐节保留 1–2 条要点，长度随有效章节数量增加，适合报告和课程；零基础讲解参考 ELI5 的“零背景、少量文字、直观图解”原则，改用短句、日常词和能形成画面的具体类比，适合第一次接触主题的读者。讲解不会使用儿童化语气，也不会用外部常识填补原文缺失的信息。只有原文存在实质性限制时才添加“限制与例外”，也不会把作者观点、推测写成已证实事实。输出只使用当前 PDF 渲染器稳定支持的 Markdown；原文没有明确结论时，模型不得替作者补出结论。

PDF 使用没有封面和目录的紧凑摘要模板。长图沿用 Tablet / Mobile 的字体、行宽与段落节奏，把分页改成一张连续图片；如果摘要长到超过 Chromium 的稳定单图高度，页面会提示改用 PDF 或 Markdown。

## API 与隐私

PDF 转换和上传 PDF 的文字提取完全在 Streamlit 应用的运行环境中完成，不调用 DeepSeek、OpenAI 或其他模型服务。网页只在点击“读取网页正文”时访问，且仅允许公开的 HTTP(S) 地址、最多读取 25 MB；摘要页单个 Markdown、TXT 或 PDF 文件最多上传 100 MB。本机、内网和带账号密码的网址会被拒绝。只有点击摘要页的“生成摘要”后，编辑器中的正文才会发送到 DeepSeek。
