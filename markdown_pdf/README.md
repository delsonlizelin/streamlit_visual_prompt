...
一个轻量的双页面 Streamlit 应用：把中文、英文或双语 Markdown 转成适合长时间阅读的 PDF，或从粘贴文本、Markdown、TXT、普通文本型 PDF 和公开文章网址生成忠实、可下载的文章摘要。两个页面使用顶部的单一按钮切换，不依赖侧边栏；页面为 Streamlit Cloud 固定工具栏保留安全区域，并针对手机窄屏调整编辑器、选项和触控按钮。
...
- 核心摘要、按章节梳理两种内容结构，以及直接摘要、零基础讲解两种可组合的讲述方式；
...
上传文件后，下载文件名默认继承原文件名；读取网页时默认继承文章标题；直接粘贴时，摘要页使用 `summary`，PDF 页面使用 `longread`。文件名不包含 Desktop、Tablet 或 Mobile 等阅读模式。原文输入始终可见，PDF 会显示已提取页数和字符数；编辑后主按钮会明确变为“重新生成摘要”，同一文件也可手动重新读取。结果直接展示，PDF 和长图导出收在渐进展开区。摘要页只使用 DeepSeek Chat Completions API。请求使用 JSON 输出，单篇上限为 30 万字符；未配置 Key 时页面仍能读取网页、上传和编辑，但生成按钮会禁用。点击生成即表示同意把当前输入发送到 DeepSeek，不再提供重复确认框。

提示词要求中性、直接和忠实：核心摘要跨章节重组信息，提供一个明确结论和 3–5 个不重复要点，适合日常阅读与转发；按章节梳理沿原文结构逐节提炼，长度随有效章节数量增加，适合报告和课程。讲述方式独立于内容结构：直接摘要保留必要术语和信息密度；零基础讲解面向没有相关背景的成年读者，补充原文能够支持的前置知识，解释术语，并展开每个主要观点的原因、过程、证据、结果和全文推理链。零基础讲解不再使用极短 ELI5 篇幅，也不会使用儿童化语气或用外部常识填补原文缺失的信息。每种结构、讲述方式和篇幅组合都有对应的中文目标字数与英文目标词数；原文信息不足时允许少于下限，不重复或虚构内容凑字数。补充要求不能覆盖忠实性、内容结构、Markdown 和 JSON 输出规则。页面只把内容结构、讲述方式和详细程度放在主流程；语言、模型、补充要求、文件名与完整 Prompt 收进“更多设置”。输出只使用当前 PDF 渲染器稳定支持的 Markdown；原文没有明确结论时，模型不得替作者补出结论。
...
摘要请求使用 Python 标准库发送，没有引入 DeepSeek SDK 或额外 AI 框架。Prompt 把原文放在可变摘要设置之前，使同一文章修改模式并重新生成时更容易复用 DeepSeek 默认开启的前缀缓存；成功请求只发送一次。仅在 HTTP 429、500 或 503 时短暂等待并重试一次，其他失败不会盲目重试。
