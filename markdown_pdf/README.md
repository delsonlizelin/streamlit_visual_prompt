...
提示词要求中性、直接和高信息密度：快速概览严格为 3–5 句无列表短文，适合先判断是否值得细读；核心摘要跨章节重组信息，提供一个明确结论和 3–5 个不重复要点，适合日常阅读与转发；按章节梳理沿原文结构逐节保留 1–2 条要点，长度随有效章节数量增加，适合报告和课程；零基础讲解补齐原文支持的最低必要背景，解释术语并串起因果，适合第一次接触主题的读者。讲解不会使用儿童化语气，也不会用外部常识填补原文缺失的信息。只有原文存在实质性限制时才添加“限制与例外”，也不会把作者观点、推测写成已证实事实。输出只使用当前 PDF 渲染器稳定支持的 Markdown；原文没有明确结论时，模型不得替作者补出结论。
